<!-- generated-by: co-agent · source: CLAUDE.md · claude-md-sha: fb57cabb9846 · generated-at: 2026-09-14 · DO NOT EDIT — edit CLAUDE.md then run /co-agent sync-context -->
...
- **AgentCore known IDs are not readiness:** keep all known catalog gateway IDs for Runtime `GATEWAYS_JSON`, pruning and ADR-017 teardown after read/update/readiness failures. A separate ready-key set gates positive target provisioning only; incomplete identity must block Runtime updates, never silently erase a domain. Preserve deployed inbound auth/protocol. Description-only failures warn; readback requires READY plus role/Lambda ARN, not exact cosmetic echo. Recovery waits may tolerate the previous failure only until a transition/deadline. Canonical contract: `docs/reference/05-agentcore.md`.
