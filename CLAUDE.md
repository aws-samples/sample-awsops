...
## 아키텍처 (v2)
- **IaC**: **Terraform** (CDK 폐기). `terraform/foundation/` 단일 루트, **partial S3 backend**(`backend.hcl`, `awsops-v2-tfstate`, `use_lockfile` — DynamoDB 없음). TF ≥1.15, provider `~>6.0`.
...
라이브 환경: 계정 `<ACCOUNT_ID>`, 도메인 `awsops-v2.atomai.click`, mgmt-vpc 재사용(`vpc-06801144309cad7dc`, 10.254.0.0/16).
...
### Terraform 규율
- 변경은 `terraform/foundation/`에서. **공유 인프라에 `-auto-approve` 금지** — saved-tfplan(`apply tfplan`)이 자동 게이트 통과. 긴 apply(CloudFront, SG)는 **컨트롤러가 실행**(서브에이전트 idle-timeout).
...
### 데이터 / 설정
- 앱 상태는 **Aurora**(node-pg). `data/*.json`(v1 패턴) 아님. 스키마는 `terraform/foundation/data/schema.sql` + `schema_migrations`.
...
terraform -chdir=terraform/foundation plan -out tfplan   # 컨트롤러가 apply tfplan (공유 인프라)
...
## Architecture (v2)
- **IaC**: **Terraform** (CDK dropped). Single `terraform/foundation/` root; **partial S3 backend** (`backend.hcl`, `awsops-v2-tfstate`, `use_lockfile` — no DynamoDB). TF ≥1.15, provider `~>6.0`.
...
Live env: account `<ACCOUNT_ID>`, domain `awsops-v2.atomai.click`, reused mgmt-vpc (`vpc-06801144309cad7dc`, 10.254.0.0/16).
...
- Change under `terraform/foundation/`. **No `-auto-approve` on shared infra** — a saved-tfplan (`apply tfplan`) passes the auto-gate. Long applies (CloudFront, SG) are **run by the controller** (subagent idle-timeout).
...
terraform -chdir=terraform/foundation init -backend-config=backend.hcl
terraform -chdir=terraform/foundation plan -out tfplan   # controller runs apply tfplan (shared infra)
...
| Aspect | v1 (`src/`) | v2 (`web/` + `terraform/`) |
|--------|-------------|-------------------------------|
| IaC | CDK | Terraform (partial S3 backend) |
