...
Dev Deploy Web also requires the private migration before promotion. Only its exact samples/dev
caller may opt the reusable migration into push events; standalone/AgentCore remain dispatch-only.
Web promotion uses the fresh build digest or a verified successful producer receipt
(`image_build_run_id` for reuse), never the mutable `web-<SHA>` tag alone.
