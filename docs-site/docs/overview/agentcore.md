...
| **Runtime 업데이트** | 멱등 provisioner(`scripts/v2/agentcore/provision.py`)로 변경 요청을 제출합니다. 갓 생성한 Gateway는 `READY` 확인 후 재실행합니다. 지속적인 `FAILED`는 원인 진단이 필요하며 자동 삭제·재생성하지 않습니다. |
