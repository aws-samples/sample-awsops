...
기본 인증으로 조회하려면 대상 클러스터에 **web 태스크 역할의 EKS Access Entry**가 필요합니다. 명시적 SA 토큰·AssumeRole 인증은 해당 인증 주체의 Kubernetes 권한을 사용하며, 계정 메타데이터 조회 설정은 별도로 필요합니다. 관리자는 조회 접근을 **등록/해제**하거나, 직접 클러스터에 적용할 수 있는 **온보딩 스크립트**를 확인할 수 있습니다. AWSops는 클러스터를 변경하지 않으며 모든 동작은 읽기 전용입니다.
...
:::info 연결 조건
기본 인증에는 **web 태스크 역할의 EKS Access Entry**가 필요하며, 명시적 SA 토큰·AssumeRole 인증도 지원합니다. 실제 조회 성공 여부와 부분 수집 안내를 함께 확인하세요. 미연결 클러스터는 온보딩 스크립트가 함께 제공되며, 등록/해제는 관리자만 수행할 수 있습니다. 표시되는 시각은 KST(Asia/Seoul) 기준입니다.
:::
