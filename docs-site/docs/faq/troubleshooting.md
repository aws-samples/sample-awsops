...
AI 진단은 웹이 직접 실행하지 않고 **비동기 워커 티어**가 백그라운드에서 생성하는 읽기 전용 리포트입니다(Light·Mid 8+1섹션(총 9) / Deep 15+1섹션(총 16)). 따라서 "응답이 없다"가 곧 "실패"는 아닙니다.
...
3. **deep + Opus 모델** — deep 진단(15+1섹션)에서 Opus 모델을 선택하면 비용 게이트가 적용되고 시간이 더 걸립니다. 빠르게 보려면 기본 Sonnet으로 Light/Mid 진단을 사용하세요.
...
AI 어시스턴트는 읽기 전용 도구(약 160개)로 라이브 AWS 데이터를 조회하고, 대화는 Aurora에 저장됩니다.
