...
:::info 멀티 어카운트와 무관
외부 데이터소스 설정은 **글로벌**입니다 — 사이드바의 AccountSelector를 바꿔도 영향이 없습니다. Prometheus/Loki 등은 AWS 계정과 1:1 매핑이 아니므로 모든 계정 사용자가 동일한 데이터소스 목록을 봅니다 (단, Departments의 `datasourceIds` 제한이 있다면 그것을 따름).
:::
