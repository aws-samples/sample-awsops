...
:::info Lazy Loading
Events 및 Write Events 탭은 클릭 시에만 데이터를 로드합니다(`eventsLoaded` 플래그). 페이지 진입 시에는 `summary` + `trailList`만 조회하고, 사용자가 Events/Writes 탭 중 하나를 클릭하면 한 번에 둘 다 로드합니다 — CloudTrail Lookup API는 호출 횟수와 응답 시간이 길어 CloudFront 30초 타임아웃을 피하기 위한 최적화입니다.
:::

:::tip 멀티 어카운트
멀티 어카운트 모드에서는 사이드바 AccountSelector로 대상 계정을 전환할 수 있고, 트레일 목록 및 이벤트에 **Account** 컬럼이 자동 추가됩니다 (AccountBadge로 별칭 + 컬러 도트 표시). 트레일 상세 패널에서도 어떤 계정 소속인지 헤더에 명시됩니다.
:::
