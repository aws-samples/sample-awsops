...
:::info 対象範囲
アカウント・リージョンを変更すると、その範囲のコスト表示を再取得します。同名クラスターは EKS カードの **Account** と **Region** で区別してください。一部失敗や取得上限に達した結果は不完全で、全リージョン（ワイルドカード）の検出も設定済み・登録済みのリージョンだけが対象です。未表示のクラスターやコストを「存在しない / 0」と解釈しないでください。
:::

**Pod 転送量（NFM）** は、ホストアカウントかつ AWSops のデプロイ先リージョンでのみ利用できます。メンバーアカウントやそれ以外のリージョンでは利用不可の理由を表示します。これは OpenCost の **Network** コストとは別のデータソースであり、NFM が利用不可でも OpenCost の Network コストが利用不可とは限りません。
