...
:::info 表示時刻
構成トポロジーは元データの取得時刻の範囲を表示し、ホスト範囲で取得時刻がない場合は最終成功時刻で補完します。時刻はブラウザーのタイムゾーンを使用し、現在の取得時刻やライブ通信を証明しません。全アカウントの集計収集状態とインベントリ取得失敗は別に表示されます。集計成功は個別アカウントの収集正常性を証明しません。
:::
