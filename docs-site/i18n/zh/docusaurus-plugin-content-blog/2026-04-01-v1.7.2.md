...
description: "Grafana 风格的外部数据源管理（7 种）、SSRF/SQL 注入防护、动态 VPC CIDR、新增数据源诊断代理。"
authors: [awsops]
...
新增了以 Grafana 风格管理 Prometheus、Loki、ClickHouse 等 **7 种外部数据源**的功能。本次发布还包含涵盖 SSRF 防护和 SQL 注入防范的安全层、**动态 VPC CIDR** 配置，以及用于诊断数据源连接问题的**专用代理**。
...
| **Query Execution** | 按类型使用相应查询语言即时执行 |
| **4 种认证** | None、Basic Auth、Bearer Token、Custom Header |
| **Credential Masking** | API 响应中的密码/令牌 → `***` 掩码 |
| **i18n** | 支持韩语 + 英语 |
...
ClickHouse 查询不会将用户输入直接插入字符串，而是使用**参数化查询**：
...
CDK 栈中的 VPC CIDR 现在可以通过**上下文参数**进行配置。
...
| `src/lib/datasource-client.ts` | HTTP 客户端（SSRF 防护 + 认证 + 查询） |
| `src/lib/datasource-registry.ts` | 7 种数据源类型注册表 |
| `src/lib/app-config.ts` | 新增 `DatasourceConfig` 类型 |
| `src/components/layout/Sidebar.tsx` | 在 Monitoring 分类中新增 Datasources |
| `infra-cdk/lib/awsops-stack.ts` | `newVpcCidr` 上下文参数 + `tgwRouteCidrs` |
| `src/app/api/ai/route.ts` | 新增 `datasource` 路由（priority 11） |

:::tip 新页面添加模式
本次数据源页面是第一个不依赖现有 Steampipe 查询、而与**外部 HTTP 端点**通信的页面。未来对接外部服务时，请参考 `datasource-client.ts` 中的 SSRF 防护 + 认证层。
:::
