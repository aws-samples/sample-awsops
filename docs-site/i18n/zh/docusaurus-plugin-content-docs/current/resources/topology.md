...
:::info 显示时刻
配置拓扑显示源数据采集时间范围；主账户范围内缺少采集时间时，以最近成功时间作为回退。时间使用浏览器时区，既不是当前获取时间，也不能证明实时流量。所有账户的汇总采集状态与资产清单读取失败分别显示。汇总成功不能证明单个账户的采集正常。 资产清单读取仅应用账户选择。EKS归属检查仅覆盖API配置区域列表中的已连接集群；其他区域和未连接集群明确标为未评估，与读取失败区分。空图也会显示响应上限。
:::
...
- 目标组、ECS 任务及子网最多读取20页、每页500行，与 EKS 共享浏览器30秒期限。每页存储行与全局采集记录/行数在同一只读 Repeatable Read 数据库快照中读取。关键类型必须提供 `consistency: "repeatable-read"` 标记，且各页成功版本稳定。旧响应、缺少标记、失败、元数据变化或仍有上限时不确认归属，不比较浏览器与数据库时钟。多页/多类型不是同一个快照，也不证明 AWS 数据完整或新鲜。
