...
- 目标组、ECS 任务及子网最多读取20页、每页500行，与 EKS 共享浏览器30秒期限。每页存储行与全局采集记录/行数在同一只读 Repeatable Read 数据库快照中读取。关键类型必须提供 `consistency: "repeatable-read"` 标记，且各页成功版本稳定。旧响应、缺少标记、失败、元数据变化或仍有上限时不确认归属，不比较浏览器与数据库时钟。多页/多类型不是同一个快照，也不证明 AWS 数据完整或新鲜。 清单及名称补充查询在每次页面加载中最多使用两个并行通道，各通道内关键类型的分页顺序执行。
- 使用集群筛选前，请检查读取/范围警告及归属不明图标。部分读取失败时仍显示成功类型。失败或未完成的读取产生空图时，仅在相同账户保留非空旧图及原有依据，并显示保留提示；正常完成的空结果会替换旧图。
- 目标的 `targetCapturedAt` 是目标组行的采集时间，不是任务/子网/Pod 的归属证据时间。成员/物化图目标和主机 ECS 快照标签均为缓存配置。代理 SQL 视图可显示 region/cluster/ECS/task 字段，却省略来源限定和目标时间，因此投影后的标签不能证明实时归属。
