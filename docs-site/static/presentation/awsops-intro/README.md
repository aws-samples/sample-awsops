...
- `merge-verify.yml`은 docs-site 또는 검증 워크플로 변경 시 **4중 게이트**를 실행한다 (누락·위반 시 필수 머지 검증 실패):
  ...
  3. **생성기 일치(프로비넌스)** — CI가 스크립트로 재빌드해 파트 목록 + 전체 아카이브(media 포함)를 diff. 손으로 바꾼 바이너리는 머지 검증 실패
...
`deploy-guide.yml` 자체는 이 덱 검증을 실행하지 않는다. 수동 게시 전에는 빌드된 파일도 검증한다:
