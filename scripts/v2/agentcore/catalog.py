...
TARGETS = {
    ...
    "prometheus-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "prometheus_query", "description": "Instant PromQL query at a single time collectionStatus ok/empty denotes complete results; partial/unknown/error or truncated output must not support an absence claim. Useful partial rows do not establish full coverage. Floating-point vector/matrix results only; scalar/string results return unsupported_result_type without sample values.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "PromQL"), "time": _p("string", "Eval time: unix/ISO (default now)")}, "required": ["query"]}},
            {"name": "prometheus_query_range", "description": "Range PromQL query over a time window collectionStatus ok/empty denotes complete results; partial/unknown/error or truncated output must not support an absence claim. Useful partial rows do not establish full coverage. Floating-point vector/matrix results only; scalar/string results return unsupported_result_type without sample values.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "PromQL"), "start": _p("string", "1h/30m or unix/ISO (default now-1h)"), "end": _p("string", "unix/ISO (default now)"), "step": _p("string", "Step seconds (default 60)")}, "required": ["query"]}},
    ...
    "tempo-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "tempo_get_trace", "description": "Fetch a trace by hex ID. tracePayloadTruncated=true with truncated=true and collectionStatus=partial is a producer-computed byte omission, not a missing trace. It supplies no spans or complete/empty proof; usable siblings may remain partial. Unmarked empty/error children remain unconfirmed.", "inputSchema": {"type": "object", "properties": {"trace_id": _p("string", "Hex trace ID")}, "required": ["trace_id"]}},
    ...
    "mimir-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "mimir_query_range", "description": "Range PromQL query. collectionStatus ok/empty denotes complete results; partial/unknown/error or truncated output must not support an absence claim. Useful partial rows do not establish full coverage. Floating-point vector/matrix results only; scalar/string results return unsupported_result_type without sample values.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "PromQL"), "start": _p("string", "1h/30m or unix (default now-1h)"), "end": _p("string", "unix (default now)"), "step": _p("string", "Step seconds (default 60)")}, "required": ["query"]}},
