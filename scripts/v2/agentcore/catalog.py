...
TARGETS = {
    ...
    "clickhouse-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "clickhouse_query", "description": "Run a read-only SQL query (SELECT/SHOW/DESCRIBE) against the connected ClickHouse; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error.", "inputSchema": {"type": "object", "properties": {"sql": _p("string", "Read-only SQL"), "max_rows": _p("string", "Max rows (<=1000)")}, "required": ["sql"]}},
            {"name": "clickhouse_tables", "description": "List tables (SHOW TABLES); collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error.", "inputSchema": {"type": "object", "properties": {}}},
            {"name": "clickhouse_describe", "description": "Describe a table's columns; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error.", "inputSchema": {"type": "object", "properties": {"table": _p("string", "Table name")}, "required": ["table"]}},
        ],
    },
    ...
    "tempo-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "tempo_search", "description": "Search traces by TraceQL over a time window. collectionStatus is ok/empty/partial/unknown/error; count_not_confirmed identifies unavailable job-count proof, not confirmed empty.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "TraceQL"), "start": _p("string", "1h/30m or unix sec (default now-1h)"), "end": _p("string", "unix sec (default now)"), "limit": _p("string", "Requested trace limit (default 20); reaching it is partial. Output above 50 traces is truncated.")}, "required": ["query"]}},
            {"name": "tempo_get_trace", "description": "Fetch a trace by hex ID. Oversized responses may retain a bounded_otlp projection with truncated=true. If no valid span fits, tracePayloadTruncated=true marks an explicit partial byte omission. Neither proves complete or empty coverage. Unmarked empty/error children remain unconfirmed.", "inputSchema": {"type": "object", "properties": {"trace_id": _p("string", "Hex trace ID")}, "required": ["trace_id"]}},
    ...
    "mimir-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "mimir_labels", "description": "List label names; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error.", "inputSchema": {"type": "object", "properties": {}}},
            {"name": "mimir_series", "description": "Find series matching a selector; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error.", "inputSchema": {"type": "object", "properties": {"match": _p("string", "Series selector")}, "required": ["match"]}},
