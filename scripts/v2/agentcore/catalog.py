...
TARGETS = {
    ...
    "inventory-read-target": {
        ...
        "tools": [
            ...
            {"name": "get_topology", "description": "Return the host-scoped materialized topology graph (nodes + edges) from Aurora. class='flow' (default) is the traffic-path graph (CF→LB→TG→target); 'infra' is resource relationships; 'trace' is observed telemetry. Omitted/null resource_id selects the whole class; a nonempty ID selects a node and its 1-hop neighbourhood before the 500-node/1000-edge caps. Inspect selection and truncation; edge truncation includes observed edge loss caused by the node cap. collection describes saved/source evidence; reader readOutcome and snapshotConsistent=false disclose failed verification or publication change. An absent snapshotConsistent is not proof of one coherent snapshot, and an empty selection does not establish an empty or healthy graph.", "inputSchema": {"type": "object", "properties": {"resource_id": _p("string", "Optional exact canonical node ID (e.g. cf:E123, alb:arn:aws:...) or exact raw ID after the first kind prefix (e.g. E123, arn:aws:...). Canonical matches take precedence; ambiguous raw IDs return candidate canonical IDs without selecting a graph. Omit or use null for an unscoped read. Supplied strings are trimmed; nonempty and at most 4096 characters. Use full canonical IDs for composite nodes."), "class": _p("string", "Graph class: 'flow' (traffic path, default), 'infra' (resource relationships), or 'trace' (observed telemetry)")}}},
    ...
    "clickhouse-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "clickhouse_query", "description": "Run a read-only SQL query (SELECT/SHOW/DESCRIBE) against the connected ClickHouse; collectionStatus ok/empty denotes complete results; partial/unknown/error or truncated output must not support an absence claim. Useful partial rows do not establish full coverage.", "inputSchema": {"type": "object", "properties": {"sql": _p("string", "Read-only SQL"), "max_rows": _p("string", "Max rows (<=1000)")}, "required": ["sql"]}},
            {"name": "clickhouse_tables", "description": "List tables (SHOW TABLES); collectionStatus ok/empty denotes complete results; partial/unknown/error or truncated output must not support an absence claim. Useful partial rows do not establish full coverage.", "inputSchema": {"type": "object", "properties": {}}},
            {"name": "clickhouse_describe", "description": "Describe a table's columns; collectionStatus ok/empty denotes complete results; partial/unknown/error or truncated output must not support an absence claim. Useful partial rows do not establish full coverage.", "inputSchema": {"type": "object", "properties": {"table": _p("string", "Table name")}, "required": ["table"]}},
        ],
    },
    ...
    "prometheus-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "prometheus_query", "description": "Instant PromQL query at a single time; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status. Supports bounded vector/matrix and scalar/string results. Native histogram output is unsupported; request float-valued results.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "PromQL"), "time": _p("string", "Eval time: unix/ISO (default now)")}, "required": ["query"]}},
            {"name": "prometheus_query_range", "description": "Range PromQL query over a time window; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status. Native histogram output is unsupported; request float-valued results.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "PromQL"), "start": _p("string", "1h/30m or unix/ISO (default now-1h)"), "end": _p("string", "unix/ISO (default now)"), "step": _p("string", "Step seconds (default 60)")}, "required": ["query"]}},
            {"name": "prometheus_labels", "description": "List label names; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status.", "inputSchema": {"type": "object", "properties": {}}},
            {"name": "prometheus_series", "description": "Find series matching a selector; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status.", "inputSchema": {"type": "object", "properties": {"match": _p("string", "Series selector e.g. up{job=\"x\"}")}, "required": ["match"]}},
    ...
    "tempo-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "tempo_search", "description": "Search traces by TraceQL. collectionStatus distinguishes complete ok/empty from partial/unknown/error. A validated synchronous HTTP 200 SearchResponse can prove empty, including protobuf-omitted empty traces with valid metrics. Job counters may be omitted; explicit unfinished jobs, HTTP 206, warnings and caps are partial. Invalid/unrecognized shape is unknown (search_response_unverified); inspected metrics alone are not completion proof.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "TraceQL"), "start": _p("string", "1h/30m or unix sec (default now-1h)"), "end": _p("string", "unix sec (default now)"), "limit": _p("string", "Integer trace limit clamped to 1..50 (default 20); reaching it is partial.")}, "required": ["query"]}},
            {"name": "tempo_get_trace", "description": "Fetch a trace by hex ID. Oversized responses may retain a bounded_otlp projection with truncated=true. If validated spans cannot fit, tracePayloadTruncated=true marks a partial byte omission. tracePayloadUnverified=true marks locally omitted unverified trace data with collectionStatus=unknown. Neither omission proves absence or complete coverage. Unmarked empty/error children remain unconfirmed.", "inputSchema": {"type": "object", "properties": {"trace_id": _p("string", "Hex trace ID")}, "required": ["trace_id"]}},
    ...
    "mimir-mcp-target": {
        ...
        "tools": [
            ...
            {"name": "mimir_query", "description": "Instant PromQL query; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status. Supports bounded vector/matrix and scalar/string results. Native histogram output is unsupported; request float-valued results.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "PromQL"), "time": _p("string", "Eval time unix/ISO (default now)")}, "required": ["query"]}},
            {"name": "mimir_query_range", "description": "Range PromQL query; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status. Native histogram output is unsupported; request float-valued results.", "inputSchema": {"type": "object", "properties": {"query": _p("string", "PromQL"), "start": _p("string", "1h/30m or unix (default now-1h)"), "end": _p("string", "unix (default now)"), "step": _p("string", "Step seconds (default 60)")}, "required": ["query"]}},
            {"name": "mimir_labels", "description": "List label names; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status.", "inputSchema": {"type": "object", "properties": {}}},
            {"name": "mimir_series", "description": "Find series matching a selector; collectionStatus is ok/empty/partial/unknown/error. Keep partial observations with caveats; never infer absence or full coverage from partial/unknown/error. Byte-bounded payload_truncated omits result data and is not empty collection; retain its partial/unknown/error status.", "inputSchema": {"type": "object", "properties": {"match": _p("string", "Series selector")}, "required": ["match"]}},
