...
TARGETS = {
    ...
    "inventory-read-target": {
        ...
        "tools": [
            ...
            {"name": "get_topology", "description": "Return the host-scoped materialized topology graph (nodes + edges) from Aurora. class='flow' (default) is the traffic-path graph (CF→LB→TG→target); 'infra' is resource relationships; 'trace' is observed telemetry. resource_id selects a node and its 1-hop neighbourhood before the 500-node/1000-edge response caps. Inspect selection for resolved/not_found/ambiguous status and truncation for incomplete responses; an empty selection does not establish an empty or healthy graph.", "inputSchema": {"type": "object", "properties": {"resource_id": _p("string", "Optional exact canonical node ID (e.g. cf:E123, alb:arn:aws:...) or exact raw ID after the first kind prefix (e.g. E123, arn:aws:...). Canonical matches take precedence; ambiguous raw IDs return candidate canonical IDs without selecting a graph. Nonempty string, at most 4096 characters."), "class": _p("string", "Graph class: 'flow' (traffic path, default), 'infra' (resource relationships), or 'trace' (observed telemetry)")}}},
