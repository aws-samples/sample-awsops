...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (process.argv[2] === 'seal') {
    ...
    } else if (['open', 'inspect'].includes(process.argv[2])) {
      ...
      if (process.argv[2] === 'inspect') {
        if (process.env.GITHUB_ACTIONS === 'true') throw new Error('Inspect plans on a private operator machine, not a shared CI runner');
        if (!process.env.PLAN_INSPECTION_DIR) throw new Error('PLAN_INSPECTION_DIR is required for confidential inspection output');
        const directory = resolve(process.env.PLAN_INSPECTION_DIR);
        mkdirSync(directory, { recursive: true, mode: 0o700 }); chmodSync(directory, 0o700);
        const path = join(directory, 'inspected.tfplan');
        writeFileSync(path, plan, { mode: 0o600 });
        try {
          writeFileSync(join(directory, 'plan.txt'), command('terraform', ['show', '-no-color', path]), { mode: 0o600 });
          writeFileSync(join(directory, 'plan.json'), command('terraform', ['show', '-json', path]), { mode: 0o600 });
        } finally { rmSync(path, { force: true }); }
        console.log(`Inspection only: confidential plan.txt/plan.json written to ${directory}; no approval or apply performed`);
      } else {
        ...
        guardPlan(json, { reviewedDeletes: process.env.REVIEWED_DELETES === 'true', manualDns: metadata.target === 'dev',
          bootstrapOrigin: process.env.REVIEWED_ORIGIN_BOOTSTRAP === 'true', enforceDeploymentPosture: true,
          taskRevisionScope: metadata.target === 'dev'
            ? { project: 'awsops-dev', region: 'ap-northeast-2', account: process.env.AWS_DEV_ACCOUNT_ID } : undefined });
    ...
    } else throw new Error('Expected seal, inspect or open');
