...
async function applyPhase(phase, variables) {
  ...
  if (pinned.digest !== hash(readFileSync(`${PRIVATE}/tfplan`)) ||
      pinned.backend !== hash(readFileSync(`${TF}/backend.hcl`)) || pinned.config !== hash(readFileSync(`${TF}/terraform.tfvars`))) throw new Error('Saved plan/config changed');
  await assertContext();
  console.log(`Applying saved ${phase} plan (no deletes, replacements or DNS writes)`);
...
async function images() {
  ...
  for (const kind of ['web', 'migration']) {
    const tag = `${kind}-${sha}`;
    let digest = await imageDigest(repo, tag);
    if (!digest) {
      if (kind === 'web') copyFileSync('CHANGELOG.md', 'web/CHANGELOG.md');
      const args = ['buildx', 'build', '--platform', 'linux/arm64', '--provenance=false', '--sbom=false', '--push', '--tag', `${uri}:${tag}`];
      if (kind === 'migration') args.push('--file', 'scripts/v2/ci/Dockerfile.migration', '.'); else args.push('web');
      await run('docker', args, { visible: true });
      digest = await imageDigest(repo, tag);
    }
    if (!digest) throw new Error(`Missing ${kind} image after build`);
    built[kind] = digest;
  }
  save('images.json', { ...built, sha });
  for (const [kind, digest] of Object.entries(built)) jobOutput(`${kind}_digest`, digest);
}
async function registerRevision(template, containerName, image) {
...
async function migrate(out, digest) {
  ...
  try {
    save('run-task.json', { cluster: out.ecs_cluster_name, taskDefinition: revision,
      ...
      clientToken: `migration-${process.env.GITHUB_RUN_ID}-${process.env.GITHUB_RUN_ATTEMPT}`,
...
async function verify(out, digest) {
  for (let attempt = 0; attempt < 100; attempt++) {
    ...
    if (failures?.length) throw new Error('ECS service lookup failed');
  ...
  throw new Error('Service did not converge to expected digest and healthy containers');
}
async function release() {
  ...
  console.log('Running private migration task');
  await migrate(out, built.migration);
  const revision = await registerRevision(out.deployment_config.web_template, 'web', `${out.ecr_web_uri}@${built.web}`);
  phase.variables.web_task_definition_arn = revision;
  phase.variables.web_desired_count = Math.max(1, phase.variables.web_desired_count);
  const rolled = await applyPhase('service', phase.variables);
