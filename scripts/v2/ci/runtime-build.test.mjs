...
test('reusable web migration preserves account and caller checks on the authorized push path', () => {
  const automatic = { ...env, GITHUB_EVENT_NAME: 'push', MIGRATION_FROM_DEPLOY_WEB: 'true',
    GITHUB_WORKFLOW_REF: 'aws-samples/sample-awsops/.github/workflows/deploy-web.yml@refs/heads/dev' };
  assert.doesNotThrow(() => verifyCaller(automatic, caller));
  assert.throws(() => checkRole({ ...automatic, MIGRATION_FROM_DEPLOY_WEB: 'false' }));
  assert.throws(() => checkRole({ ...automatic, GITHUB_WORKFLOW_REF: 'other' }));
  assert.throws(() => verifyCaller(automatic, { ...caller, Account: '999999999999' }));
});
