...
test('an unavailable encryption key fails and retains diagnostics in a private directory outside cleanup', () => {
  const f = fixture('');
  try {
    const result = spawnSync(process.execPath, [script], { cwd: f.root, env: f.env, encoding: 'utf8' });
    ...
    rmSync(join(f.root, 'terraform'), { recursive: true });
    assert.equal(readFileSync(join(output.retained_dir, 'tfplan.log'), 'utf8'), 'private-diagnostic-marker');
    assert.equal(statSync(join(output.retained_dir, 'tfplan.log')).mode & 0o777, 0o600);
  } finally { rmSync(f.root, { recursive: true, force: true }); }
});
