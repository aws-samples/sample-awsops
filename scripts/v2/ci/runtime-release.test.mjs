...
for (const type of ['rds', 'cloudfront']) test(`release preserves ${type} degradation after a successful owned probe`, async () => {
  ...
  }, { authResult: { status: 'ok', mode: 'verify', catalog_types: 2, workers: 2, collection } });
});
test('release refuses missing, fabricated or foreign collection proof', async () => {
  for (const collection of [undefined, { ...currentCollection, completeness: 'complete' },
    ...
    { ...currentCollection, status: 'degraded',
      degraded_types: [{ type: 'cloudfront', status: 'succeeded', unknown_attributes: false }] }]) await withFixture(async f => {
...
function withFixture(check, overrides = {}) {
  const f = fixture(overrides);
  let result;
  try {
    result = check(f);
  } finally {
    // Keep synchronous checks synchronous; async checks clean up after settling.
    if (!(result instanceof Promise)) f.cleanup();
  }
  return result instanceof Promise ? result.finally(f.cleanup) : result;
}
...
test('the own CloudFront probe waits through busy/superseded without another full fan-out', () => withFixture(async f => {
...
test('catalog admission retries only confirmed throttling', () => withFixture(async f => {
...
test('a long busy probe can finish and retry without an arbitrary call-count limit', () => withFixture(async f => {
...
test('uncertain CloudFront delivery is never retried or accepted as readiness', () => withFixture(async f => {
  let probes = 0;
...
test('collect binds running web, code hash, canonical coverage and the fresh own-workload probe', () => withFixture(async f => {
  const now = Date.now();
...
test('identity, wrong task role/revision/image/architecture and Lambda code failures block invocation', async () => {
  const r = withFixture(f => f.responses);
  const changes = [
...
test('prepare uses ordinary authenticated account preparation before backend boot, without Lambda access', () => withFixture(async f => {
...
test('capture persists only validated deployment metadata in the credential directory', () => withFixture(f => {
...
test('disabled runtime features fail capture before any deployment contract or AWS call', () => withFixture(f => {
  const value = deployment();
...
test('capture refuses unsafe credential modes and existing symlink targets', () => {
  for (const unsafe of ['mode', 'symlink']) withFixture(f => {
...
test('release freshness starts before the owned probe and is not reset by its response', () => withFixture(async f => {
