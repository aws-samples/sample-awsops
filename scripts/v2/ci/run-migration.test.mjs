...
import { runMigration, cleanupMigration, selectProject, validateContext } from './run-migration.mjs';
...
test('automatic migration requires the opted-in exact development web caller', () => {
  const current = { ...fixture().context, event: 'push', fromDeployWeb: 'true',
    workflowRef: 'aws-samples/sample-awsops/.github/workflows/deploy-web.yml@refs/heads/dev' };
  assert.doesNotThrow(() => validateContext(current));
  for (const change of [{ fromDeployWeb: undefined }, { fromDeployWeb: 'false' },
    { workflowRef: '' }, { workflowRef: current.workflowRef.replace('deploy-web', 'other') },
    { ref: 'refs/heads/main' }, { repository: 'other/repo' }, { event: 'pull_request' }]) {
    assert.throws(() => validateContext({ ...current, ...change }));
  }
});
