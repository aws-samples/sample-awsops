<!-- generated-by: co-agent · source: CLAUDE.md · claude-md-sha: 2148aac25bdc · generated-at: 2026-09-14 · DO NOT EDIT — edit CLAUDE.md then run /co-agent sync-context -->
...
- Dev Deploy Web requires private migration success before promotion. Standalone/AgentCore
  migrations remain dispatch-only; only the exact samples/dev Deploy Web caller may explicitly
  opt into push events. Account, role, current source and running digest checks still apply.
- `v2/ci_web_image.py` uses the fresh build digest or a verified successful same-repository,
  branch, source and stack producer receipt. Reuse needs `image_build_run_id`; missing/expired
  receipts require rebuilding. Authenticate the artifact and current attempt before ECR reads.
  Mutable `web-<SHA>` tags are not approval evidence; see `test_ci_web_image.py`.
