...
Controller collection contention: one dispatch-anchored 20-minute deadline includes login/DB. Wait 420 seconds and 60 seconds without ledger progress or running/missing rows before retrying stale terminal types. Synchronous retries are at most four concurrently/eight total, batches 60 seconds apart, and require a full 450-second remaining budget. Busy is distinct from success; successful RPCs still require fresh complete ledger evidence. Initial dispatch retries only confirmed throttling within 450 seconds. The workflow gate is 45 minutes; standalone smoke remains ten minutes. Zero unknowns remains required; the scheduler is unchanged.
