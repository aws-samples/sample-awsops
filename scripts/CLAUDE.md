...
- `v2/ci/run-migration.mjs` — development deployment controller used by
  ...
  Standalone/AgentCore calls remain dispatch-only. Only the exact samples/dev Deploy Web
  caller can explicitly opt into push events; its rollout requires migration success.
- `v2/ci_web_image.py` — promote web images by a fresh build digest or an authenticated
  GitHub artifact from a successful same-repository/branch/source/stack Deploy Web producer.
  Reuse requires `image_build_run_id`; expired/missing receipts require a fresh build.
  Validate the artifact digest and current run attempt before reading ECR by image digest.
  The mutable per-source tag is descriptive, not proof. Tests: `test_ci_web_image.py`.
