...
| `/api/inventory/summary` | GET | Type/category counts and security splits honor account/region filters. `?view=collection` returns only `{ collection }`, skipping inventory aggregations. Its `scope=aggregate`, `configured`, `readOk` and sanitized `runs` describe the collector-wide ledger; account/region filters do not narrow it or establish per-account health. Missing, failed and unknown evidence remain explicit. | verifyUser |
