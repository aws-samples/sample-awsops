...
| `/api/inventory/summary` | GET | Default resource counts honor account/region filters. `?view=collection` returns only the sanitized aggregate collection ledger and skips dashboard aggregation. Its scope remains collector-wide; it does not establish per-account health. | verifyUser |
