...
Chat reads current catalog and Agent Space policy per turn. A failed final custom
agent-enable query leaves an explicit custom pin unavailable without invocation.
Automatic routing removes custom candidates, independently resolves built-in routing,
and displays/persists that fallback notice. Product-help routing skips this final custom
query even when keywords match. Successful explicit deny-all remains deny-all; this
availability handling does not add tool permissions. Initial catalog-read failures retain
the existing SAMPLE unavailable response except for independently resolved built-in pins.
