...
owner disclosure and shared-VPC visibility limits, bounded reads and caching of
complete results. These configuration observations remain read-only under ADR-005
and do not establish reachability.
...
| VPC connectivity | [vpc-connectivity.md](vpc-connectivity.md) | `web/lib/vpc-connectivity.ts`, `web/app/api/vpc-connectivity/route.ts`, `web/components/inventory/VpcConnectivitySection.tsx` |
