...
- 소스: `agent/agent.py`(8 섹션 라우팅 + registry-agnostic 실행 + egress MCP substrate), `scripts/v2/agentcore/{catalog.py,provision.py}`(9 게이트웨이 프로비저닝 + Memory + Code Interpreter), `terraform/foundation/ai.tf`(`agentcore_enabled` 게이트 + SSM).
