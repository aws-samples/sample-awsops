...
웹은 고정된 Grafana TraceQL 파서로 문법을 검사하고, 관측된 사용자 속성·호환 타입·완전한 긍정 요청 형식(예: `HTTP 500 응답 스팬`)에서 관측된 표준 HTTP 상태 조건을 확인한다. 이 의미 검사는 초안이 표준 HTTP·서비스 속성만 참조할 때 적용된다. 부정·범위·추가 문맥이 있는 일반 문장과 비표준 HTTP 속성의 의미는 모델과 사용자가 검토한다. 문법·스키마·필터 오류가 있으면 모델에 **한 번만 수정 요청**한다(최대 두 번 생성). 수정된 결과도 실패하면 생성 API가 502를 반환하며 Tempo 검색은 실행하지 않는다. `SCHEMA_REQUIRED` 응답이나 이름 수집이 제한된 상태의 미관측 속성은 이 수정 단계를 거치지 않고 스키마 안내로 반환된다.

The web app checks syntax with a pinned Grafana TraceQL parser, then observed custom names, compatible types, and standard HTTP-status predicates for complete affirmative request templates (for example, `HTTP 500 응답 스팬`) when standard keys were observed and the draft references only standard HTTP/service attributes. Negation, ranges, extra context, and nonstandard HTTP-field semantics remain model/user review tasks. A syntax, schema, or filter error gets **one correction request** (at most two generations). If that result also fails, the generation API returns 502 without running a Tempo search. `SCHEMA_REQUIRED`, and an unobserved attribute when name discovery is limited, go directly to schema guidance without a correction attempt.
