...
타입 조회는 `maxStaleValues` 조기 종료를 사용하지 않습니다. 반복된 문자열 값 뒤에 나오는 숫자 타입을 놓치지 않도록 하며, 32개 값 제한과 짧은 HTTP 제한 시간은 유지합니다.

Type discovery does not request `maxStaleValues` early termination, so repeated string values do not prevent discovery of a later numeric type. The 32-value cap and short HTTP deadline remain.
