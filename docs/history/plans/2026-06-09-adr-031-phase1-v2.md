...
**Modify:**
- `terraform/foundation/data/schema.sql` — add 4 catalog tables + triggers + migration v2 + built-in seed (idempotent).
- `terraform/foundation/workload.tf` — add `ADMIN_GROUP` + `SSM_ADMIN_EMAILS_PARAM` env to the web container (so the admin gate has config).
- `terraform/foundation/ai.tf` — add an SSM parameter `/ops/${project}/admin_emails` (StringList, empty default), gated like the other ops params.
...
In `terraform/foundation/ai.tf`, add (near the other `/ops/${var.project}` SSM params; this one is NOT gated by `agentcore_enabled` — the admin gate is always-on):
...
In `terraform/foundation/workload.tf`, add to the web container `environment` list (after `SSM_RUNTIME_ARN_PARAM`):
...
```bash
git add terraform/foundation/ai.tf terraform/foundation/workload.tf
...
- Paths are v2: `web/lib/*`, `web/app/api/customization/route.ts`, `web/app/customization/page.tsx`, `terraform/foundation/data/schema.sql`, `agent/agent.py`. No `src/`, no `data/config.json`, no `/awsops` fetch prefix (fetch `/api/customization`). ✅
