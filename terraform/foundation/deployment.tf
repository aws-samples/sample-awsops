...
variable "web_image_digest" {
  type        = string
  default     = ""
  description = "Verified CI web image digest; empty preserves legacy web-latest behavior."
  validation {
    condition     = var.web_image_digest == "" || can(regex("^sha256:[a-f0-9]{64}$", var.web_image_digest))
    error_message = "Expected an immutable sha256 image digest."
  }
}
...
resource "aws_ecs_task_definition" "migration" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "SQL_READER_SYNC_MODE", value = var.agentcore_enabled ? "secret" : "disabled" },
