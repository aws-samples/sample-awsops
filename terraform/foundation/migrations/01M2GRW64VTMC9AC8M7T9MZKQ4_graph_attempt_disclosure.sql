-- since: 0.9.0
-- Add bounded count and unattempted-source metadata to the existing reader view.
-- Existing grants and older writer/reader compatibility are preserved.
...
SELECT account_id, class, status, attempted_at, captured_at,
  (
    ...
    WHERE (k = ANY(ARRAY['windowStartMs','windowEndMs','nodeDrops','edgeDrops',
       ...
       OR (k = ANY(ARRAY['retainedPrevious','infraUnavailable','inputTruncated','graphTruncated','sourceAttempted'])
