...
- Deployment dependency readiness: authenticated probes verify the actual web-role account and fresh AgentCore SSM reads, then require a nonce-bound runtime response proving curated inventory access, a known fresh CloudFront record and a bounded model call. With an explicit private runtime configuration, the smoke utility checks collection and owned Lambda/Fargate completion. Every dev Deploy Web release now requires the controller-generated runtime configuration and full evidence; health, login or enqueue acknowledgement alone cannot pass. Only the owned CloudFront probe requires post-marker collection with zero unknowns; the deployed catalog requires last success within thirty minutes, disclosing newer partial/failed runs and unknown attributes as degraded with completeness unknown. The dev profile creates only the verifier group and conditional managed-demo membership, without admin/IAM grants. Manual collect-runtime separates host preparation from full verification. The billed probe is restricted to administrators or deployment-verifiers, with one in-flight call and a per-process cooldown. Manual development audits separately report scoped runtime/collector status, schedule metrics and SQL-reader counts/timestamps under restricted sessions, without claiming full readiness or complete collection.
  AgentCore-off web tasks explicitly set `SSM_RUNTIME_ARN_PARAM` empty, suppressing runtime-ARN discovery and the status page's runtime-ARN SSM lookup.
...
- 배포 의존성 검증: 인증된 요청으로 실제 웹 역할의 계정과 최신 AgentCore SSM 조회를 확인하고, nonce로 연결된 런타임 응답에서 지정 인벤토리 도구 접근·알려진 최신 CloudFront 레코드·제한된 모델 호출의 성공을 검증합니다. 명시적 비공개 런타임 설정을 전달하면 스모크 도구가 수집과 사용자 소유 Lambda·Fargate 완료를 검사합니다. 모든 dev Deploy Web 배포가 controller의 전체 검증을 필수로 수행하며 health·로그인·작업 접수 응답만으로 통과하지 않습니다. 자체 CloudFront probe는 배포 기준 시각 이후 수집과 미확인 속성 0개를 요구하며, 배포된 카탈로그는 30분 이내 최근 성공을 검사합니다. 이후 partial/failed 실행과 미확인 속성은 degraded로 공개하고 완전성은 unknown으로 유지합니다. dev 프로필은 검증 그룹과 조건부 관리 demo 멤버십만 생성하며 admin·IAM 권한을 부여하지 않습니다. 수동 collect-runtime은 호스트 준비와 전체 검증을 구분합니다. 유료 probe는 관리자·deployment-verifiers로 제한하고 프로세스별 단일 실행·호출 간격을 적용합니다. 별도 수동 개발 감사는 제한된 세션으로 런타임·수집기 상태, 스케줄 지표와 SQL reader의 개수·시각을 보고하며 전체 준비 상태나 완전한 수집을 확정하지 않습니다.
  AgentCore가 꺼진 웹 task는 `SSM_RUNTIME_ARN_PARAM`을 명시적으로 비워 런타임 ARN 검색과 상태 페이지의 런타임 ARN SSM 조회를 생략합니다.
