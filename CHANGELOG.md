...
- Home dashboard: a Monthly Cost Impact (est.) list — 30-day resource-count change × a static per-type unit-cost heuristic, sorted by |impact| (top 8), explicitly labeled as a heuristic rather than billing data; fed by its own fixed 35-day trend fetch (visible on the default view), including fully-removed types' savings, hidden when the latest snapshot is stale, when the account/region scope is narrowed, or when the latest day is missing a weighted type the baseline has (a partial sync fan-out), and excluding no-baseline/no-weight types rather than showing $0.
...
- IAM-related security findings are now admin-only: the Security page's "IAM Users without MFA" check is withheld from non-admin users at the API (`/api/security` omits it from checks/summary/findings) and its tile/tab disappears from the page — the same admin gate the IAM inventory types (`iam_user`/`iam_role`) already enforced. Admins are the Cognito `admins` group or the SSM admin-email allowlist.
...
- 홈 대시보드: '월 비용 영향 추정' 리스트 — 30일 리소스 수량 변화 × 타입별 정적 단가 휴리스틱, |영향| 내림차순 상위 8, 청구 데이터가 아닌 근사임을 명시; 전용 35일 추이 조회로 기본 화면에서도 표시, 완전히 제거된 타입의 절감도 포함, 최신 스냅샷이 오래됐거나 계정/리전 스코프가 좁혀졌거나 기준일에 있던 가중치 타입이 최신일에 누락(부분 sync fan-out)이면 숨김, 30일 기준값·단가 항목 없는 타입은 $0로 표시하지 않고 제외.
...
- IAM 관련 보안 findings를 admin 전용으로 전환: 보안 페이지의 "IAM Users without MFA" 점검이 비admin 사용자에게는 API(`/api/security`가 checks/summary/findings에서 제외)와 페이지(타일/탭 미표시) 모두에서 보이지 않는다 — IAM 인벤토리 타입(`iam_user`/`iam_role`)에 이미 적용돼 있던 것과 동일한 admin 게이트. admin은 Cognito `admins` 그룹 또는 SSM admin 이메일 허용목록이다.
