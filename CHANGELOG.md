...
- Web framework security update: Next.js 15.5.25 and React 19 use async request parameters and cookies while preserving authentication, ownership checks, redirects and standalone deployment. Upgrade Vitest/Vite and patched transitive dependencies.
...
- Inventory scope changes: resource pages, category summaries and dashboard counts wait for the saved account/region selection before loading. Switching scope clears the previous view, and late responses or refresh completions cannot overwrite the current selection. Category summaries use the same scope as their resource pages; loading and failed reads remain distinct from a successful empty result.

- Custom-agent policy enforcement: persist restriction history through skill edits and removal, qualify gateway tools, and reject reserved routing names. Initial or final policy outages deny custom candidates while keeping built-in routing and product help usable; fallback is disclosed and saved. Admin forms retain loaded policy and block saving until a failed reload recovers. Apply the additive policy-history migration before updating web/runtime, review existing names and saved caps, and reattach explicit grants to restore restricted tools. Instruction-only integration grants never add gateway tools; successful empty intersections remain deny-all.
...
- PR review pipeline: two independent comprehensive Codex/Claude reports cover L2–L5, followed by the existing chair and fallback. Each vendor must explicitly declare all four lenses complete; missing responses or coverage block review. Complete filtered diffs must fit 6,000 lines and 128 KiB; incomplete image evidence or omitted source blocks before model credentials/calls, replacing misleading partial reviews with a fixed input diagnostic. Missing panel coverage skips chair calls but publishes bounded, scrubbed surviving observations without a code-safety claim. Each comprehensive report allows 60,000 bytes, the panel bundle 120,000 bytes and the actual chair stdin 256 KiB. Code/contract/prose ordering, the 900-second chair budget and fast-fail retry remain. Oversized panel reports cannot silently lose blocking findings during synthesis. Existing model selections, per-call timeouts, credential renewal and the 90-minute job bound remain. Lockfile change metadata remains visible when generated contents are omitted. A SHA-pinned recovery label selects a commit, while a GitHub protected environment requires a designated reviewer before execution; exact environment/ref trust subjects replace coarse PR/repository trust. Automatic CI scripts come from the trusted default commit, model context comes from the target base, and stale verdicts are not published. Changed static PNGs are staged from immutable HEAD Git blobs with bounded read-only manifests for every panelist and the chair; BASE pixels remain historical context, and unavailable required image evidence fails coverage without bypassing findings or review gates. Bounded report validation requires explicit completed image coverage from every cell and the chair when images are staged; missing or failed declarations override PASS and are labeled as incomplete review, while quoted examples do not count. Static PNG/WebP/ICO assets use a pinned bounded decoder with source/render lineage and a 32-file limit. Decorated failure declarations remain blocking, while image failure preserves response presence and unusable report output gets its own diagnosis. Discarded invalid chair output without a coverage declaration may recover, while declared and panel failures remain blocking. Safe path diagnostics cannot introduce environment/output keys, and final gate reasons are shell data. Safe prompt labels accompany hash-checked Codex image attachments and Claude Read paths; no tool privileges are added. See `docs/runbooks/pr-review-head-images.md`. A separate manual dev-only diagnostic uses the existing review role to verify one authenticated Read of a private synthetic image outside the working directory, publishing only bounded proof and preserving every review gate.
...
- EKS 개요: 접이식 클러스터/VPC facet 필터 — 멀티 선택 클러스터·VPC 칩(VPC 칩에 클러스터 수 표시), 활성 필터 배지, 전체 해제, filtered/total 카운터 — 클러스터 카드와 하단 fleet 패널을 함께 좁힘. 계정·리전 변경 시 EKS 목록과 플릿 데이터를 갱신하고 이전 선택의 늦은 응답을 반영하지 않습니다. 교차 계정 등록은 선택한 클러스터를 검증하고 후속 조회에도 계정·리전 정보를 유지하여 동명 클러스터를 구분하며 기존 호스트 등록과 호환됩니다. 조회 범위 제한과 부분 실패를 명시하고 호스트 계정 결과로 조용히 대체하지 않습니다. 탐색 응답의 최상위 `region`은 조회 대상이 하나일 때만 제공하며, 멤버 계정·기본 리전 외 클러스터의 NFM 파드 전송량은 지원하지 않음을 명시합니다. 기본 멤버 Kubernetes 토큰과 Access Entry 확인에는 등록된 멤버 역할을 사용하여 호스트 인증 정보를 분리합니다. 멤버 클러스터에 호스트 역할만 등록했던 환경은 해당 멤버 역할의 접근 설정이 필요합니다. 공유 멤버 역할은 Secrets 조회가 가능한 AdminView 대신 View와 노드 전용 읽기 RBAC를 사용하며, 계정 비활성화·삭제 후에도 관리자가 저장된 등록·인증 정보를 정리할 수 있습니다.
...
- 웹 프레임워크 보안 업데이트: Next.js 15.5.25와 React 19의 비동기 요청 파라미터·쿠키 API를 적용하고 인증·소유권 검사·리다이렉트·standalone 배포를 유지합니다. Vitest/Vite와 취약한 전이 의존성을 패치 버전으로 올립니다.
...
- 인벤토리 범위 전환: 리소스 화면·카테고리 요약·대시보드 개수는 저장된 계정·리전 선택을 복원한 뒤 조회한다. 범위를 바꾸면 이전 화면 상태를 초기화하고 늦은 응답이나 새로고침 완료가 현재 선택의 결과를 덮어쓰지 못하게 한다. 카테고리 요약도 리소스 화면과 같은 범위로 조회하며, 로딩·조회 실패와 정상적인 빈 결과를 구분한다.

- 커스텀 에이전트 정책 적용: 스킬 수정·삭제 뒤에도 제한 이력을 유지하고 게이트웨이 도구 식별자와 예약 이름을 검증한다. 초기·최종 정책 조회 실패는 커스텀 후보를 차단하되 기본 라우팅·제품 도움말은 유지하며 대체 경로를 표시·저장한다. 관리자 폼은 이전 정책 값을 보존하고 조회 복구 전까지 저장을 차단한다. 웹·런타임 업데이트 전에 추가 정책 이력 마이그레이션을 적용하고 기존 이름·허용 목록을 점검해야 한다. 도구 복구는 명시적 권한을 다시 연결해야 하며, 지침 전용 에이전트의 연동 권한은 게이트웨이 권한을 추가하지 않고 빈 교집합은 전체 거부를 유지한다.
...
- PR 리뷰 파이프라인: Codex와 Claude가 각각 L2–L5 전체를 한 번에 검토하고 기존 chair와 fallback이 종합한다. 두 모델 모두 네 항목의 완료를 명시해야 하며 응답·검토 범위 누락은 차단한다. 전체 필터링 diff는 6,000줄과 128 KiB 이내여야 한다. 이미지 증거 누락이나 소스 생략은 모델 인증·호출 전에 입력 오류로 차단하여 부분 검토의 오판을 방지하고, 패널 검토가 미완료이면 chair를 호출하지 않고 크기를 제한하고 비밀정보를 제거한 생존 보고서를 미판정 관찰로 게시한다. 종합 보고서별 60,000바이트, 패널 묶음 120,000바이트, 실제 chair 입력 256 KiB를 제한한다. 코드·계약·산문 순서, chair 900초와 빠른 실패 재시도는 유지한다. 긴 패널 응답을 잘라 지적 사항이 사라진 상태로 통과시킬 수 없다. 모델 선택, 호출별 시간 제한, 인증 갱신과 전체 90분 제한은 유지한다. 생성 내용을 생략한 lockfile도 변경 메타데이터는 표시한다. SHA 고정 복구 라벨은 커밋을 선택하고, GitHub 보호 환경은 실행 전에 지정 리뷰어 승인을 요구한다. 정확한 환경·브랜치 신뢰 주체가 포괄 PR·저장소 신뢰를 대체한다. 자동 CI 코드는 신뢰된 기본 브랜치 커밋에서, 모델의 검토 맥락은 대상 기준 커밋에서 읽으며 오래된 HEAD의 판정은 게시하지 않는다. 변경된 정적 PNG는 불변 HEAD Git blob에서 크기를 제한한 읽기 전용 파일로 준비하고 같은 매니페스트를 모든 패널과 chair에 전달한다. 기준 커밋의 이미지는 과거 맥락으로만 사용하며, 필요한 이미지 증거를 읽을 수 없으면 지적 사항이나 리뷰 게이트를 우회하지 않고 검토 범위 실패로 처리한다. 이미지가 준비된 경우 제한된 크기의 보고서에서 모든 패널과 chair의 명시적인 이미지 검토 완료 선언을 확인한다. 누락·실패 선언은 PASS보다 우선하여 미완료 검토로 표시하고 인용된 예시는 판정 선언으로 세지 않는다. 정적 PNG/WebP/ICO는 고정된 디코더와 입력·출력 해시 추적, 파일 32개 제한으로 처리한다. 설명이 붙은 실패 선언도 차단하며, 이미지 실패와 응답 존재 여부를 구분하고 읽을 수 없는 보고서는 별도로 진단한다. 검토 범위 선언이 없는 잘못된 chair 응답은 재시도로 복구할 수 있지만, 명시된 실패와 패널 실패는 계속 차단한다. 안전한 경로 진단은 환경·출력 키를 만들 수 없고 최종 게이트 사유는 셸 데이터로 전달한다. 안전한 프롬프트 경로 표기와 함께 해시를 확인한 Codex 이미지 첨부 및 Claude Read 경로를 전달하며 도구 권한은 추가하지 않는다. `docs/runbooks/pr-review-head-images.md` 참고. 별도의 dev 전용 수동 진단은 기존 리뷰 역할로 작업 디렉터리 밖의 사설 합성 이미지를 인증된 Read로 한 번 읽었는지 검증한다. 제한된 증명만 게시하며 모든 리뷰 게이트는 유지한다.
