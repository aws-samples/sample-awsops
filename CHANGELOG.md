...
- Service/network topology correlation foundation; the integrated service/network UI is not wired yet. The existing topology page now supplies subnet inventory and the canonical scoped EKS producer. Configured-target matching requires region/VPC evidence and RUNNING ECS claimants; stopped tasks and manual non-pod Service endpoints do not contest ownership, while unknown task states and unresolved pod references remain unverified. All name-only trace workload associations, using monitor or configured-cluster names, are context rather than identity; endpoint-to-scoped-configuration identity remains distinct. Snapshot agreement is not independent proof of live identity. Unreadable EKS inventory invalidates its known region/VPC scope while unrelated scopes remain usable; unknown scope or listing limits invalidate the full map. Contested scoped keys remain blocked across ECS/EKS sources. Host live identity checks are limited to the reported EKS region; other regions retain separately labeled, unverified contextual candidates with a coverage banner and no exclusive cluster filtering. Available configuration capture times accompany identity metadata; unknown times remain null without temporal-equivalence claims. Raw member/materialized flow metadata is tagged as cached configuration, excluded from exclusive E2E identity proof. The SQL-reader projection omits per-target qualifiers/times; agent flow labels remain configuration context. The shared deadline aborts all EKS reads; incomplete global sweeps retain cached display rows without exclusive ownership. Ownership-critical ECS/subnet reads paginate within bounds; failures, changed pages and remaining caps keep visible incomplete-inventory reasons. Network query windows, failed/capped batch coverage and service collection quality remain explicit, with missing coverage marked unknown. Overview, focus and search retain complete observed connection groups within node/edge limits; configuration, service and network evidence stay distinct. Partial inventory reads retain successful types; a failed refresh preserves previous same-account results and their original provenance banners with an explicit notice. Construct focus/search includes each connection's endpoints without treating shared constructs as transit.
...
- 서비스·네트워크 토폴로지 상관관계 기반을 추가하며, 통합 서비스·네트워크 UI는 아직 연결되지 않았습니다. 기존 토폴로지 페이지에는 서브넷 인벤토리와 범위를 검증하는 공통 EKS 수집기를 연결합니다. 구성 대상 연결에는 리전·VPC 근거와 RUNNING 상태의 ECS 후보가 필요합니다. 종료된 태스크와 Pod가 아닌 수동 Service 엔드포인트는 소유권 충돌로 보지 않으며, 미확인 태스크 상태와 확인되지 않은 Pod 참조는 미확인으로 유지합니다. 모니터나 구성 클러스터 이름만으로 연결한 모든 트레이스 워크로드는 식별 근거가 아닌 문맥으로 표시하며, 엔드포인트와 범위가 일치하는 구성 대상의 식별 연결은 별도로 유지합니다. 스냅샷의 일치는 실제 시점의 식별 정보를 독립적으로 증명하지 않습니다. 읽을 수 없는 EKS 인벤토리는 확인된 리전·VPC 범위만 제외하고 다른 범위는 유지하며, 범위 미확인이나 목록 제한은 전체 맵을 제외합니다. 상충하는 범위별 키는 ECS·EKS 양쪽에서 확인 불가 상태로 유지합니다. 호스트 식별 검증은 보고된 EKS 리전으로 제한합니다. 다른 리전의 후보는 별도 문맥 이름과 범위 경고로 보존하며 배타적 클러스터 필터나 식별 근거로 사용하지 않습니다. 확인된 구성 수집 시각은 식별 메타데이터에 전달하고 미확인 시각은 null로 유지하며 관측 시점과의 동등성을 주장하지 않습니다. 멤버 계정과 저장된 흐름의 원본 메타데이터에는 캐시된 구성 표시를 남기고 배타적 E2E 식별 근거에서 제외합니다. SQL-reader 투영에는 대상별 한정자와 수집 시각이 없으므로 에이전트의 흐름 이름은 구성 문맥으로만 해석합니다. 공유 제한 시간이 지나면 모든 EKS 읽기를 중단하며, 미완료 전역 수집의 캐시 행은 독점 소유권을 주장하지 않고 표시합니다. ECS·서브넷은 정해진 범위에서 페이지를 이어 읽으며 조회 실패·변경·남은 상한은 인벤토리 불완전 사유로 표시합니다. 네트워크 조회 시간, 실패·상한 도달 범위, 서비스 수집 품질을 보존하고 누락된 범위는 미확인으로 구분합니다. 전체 보기·포커스·검색에서 노드·관계 한도 내 연결 전체를 유지하며 구성·서비스·네트워크 근거를 구분합니다. 일부 인벤토리 조회 실패 시 성공한 타입은 유지하고 전체 새로고침 실패 시에는 같은 계정의 이전 결과와 당시 근거 상태 배너를 유지하고 이전 결과임을 표시합니다. 공유 구성 요소의 포커스·검색은 각 연결의 엔드포인트를 포함하되 구성 요소를 경유한 통신으로 단정하지 않습니다.
