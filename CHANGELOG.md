...
- VPC connectivity: `/inventory/vpc` adds an on-demand, account/region-scoped peering and TGW attachment viewer, a section shortcut and resource-graph navigation, with peer identity/state, route-table associations and localized guidance. Discloses the 500-row VPC choice cap and incomplete reads; configuration does not prove reachability. Requires a reviewed saved-plan apply for the region-conditioned `ec2:DescribeVpcPeeringConnections` grant; reuses the existing TGW read permission.
...
- VPC 간 연결: `/inventory/vpc`에 사용자 클릭으로 계정·리전 범위의 피어링과 TGW 어태치먼트를 조회하는 화면, 섹션 바로가기와 리소스 그래프 이동을 추가합니다. 상대 VPC 식별 정보·상태, 라우트 테이블 연결과 다국어 안내를 제공하며, VPC 선택 목록 500행 상한과 불완전한 조회를 명시합니다. 연결 구성만으로 실제 통신 가능 여부를 확정하지 않습니다. 리전 조건이 적용된 `ec2:DescribeVpcPeeringConnections` 권한은 검토된 저장 계획으로 apply해야 하며, 기존 TGW 읽기 권한을 재사용합니다.
