...
- Scoped topology ownership and inventory evidence. The existing topology page and materializer supply subnet inventory for region/VPC-scoped targets. ECS candidates require RUNNING; EKS candidates require an independently listed unique Pending/Running pod with an assigned IP and valid endpoint read. STOPPED/DELETED tasks and Succeeded/Failed pods do not claim reused addresses; unknown phases/states and unresolved pod references remain unverified. Manual non-pod Service entries do not contest or rename pods across namespaces. Unreadable EKS scopes block every matching IP while unrelated scopes remain usable; unknown scope or listing limits block the full map. Host checks cover only the reported EKS region. Member/all-account views explicitly disclose unqueried EKS evidence on the banner and targets. An IP shared by two clusters in one region/VPC is withheld even when workload names match; other scoped addresses remain independent. Other-region candidates retain separate unverified labels, details and a coverage banner without exclusive cluster filtering. Member/materialized labels remain cached configuration, not live ownership. Target nodes expose targetCapturedAt only for the target-group row, never as task/subnet/pod ownership time; invalid or missing values stay unknown. Every host ECS snapshot label is cached configuration too. SQL-reader can expose bare region/cluster/ECS/task identifiers while omitting VPC/subnet, ownership_evidence and targetCapturedAt; those identifiers do not establish live ownership. Target-group/ECS/subnet reads keep bounded paging, stable global sweep metadata and a shared abort deadline with EKS. Each inventory page and its global ledger/count use a read-only Repeatable Read database transaction with an explicit consistency marker, required by critical paging; no browser-clock cutoff is used. Separate pages/types are not one snapshot or proof of freshness. Partial/failed/running sweeps retain cached rows without exclusive ownership. Partial inventory reads preserve successful types; a failed/incomplete refresh that builds an empty graph retains prior nonempty same-account configuration and its provenance notices. A complete empty load clears it normally. Snapshot agreement never independently proves live identity.
...
- Configuration topology evidence: ECS/EKS IP labels require independently corroborated region/VPC, subnet or pod evidence. The page waits for its saved account scope, isolates late responses, and discloses failed or capped inventory reads. Aggregate account-sweep status is labeled under every scope, separately from read failures and unknown per-account health; running syncs and ordinary ambiguous pod IPs are not failures. Only RUNNING ECS tasks establish ownership, so stopped tasks cannot claim or poison reused IPs. EKS ownership is attempted only for a host-only scope within the API's configured region, with other regions and listed unconnected clusters explicitly unassessed; onboarding gaps remain separate from failed/partial reads. Inventory uses account selection only, and response caps remain visible even on empty graphs. Capture/last-success clocks use source evidence and viewer-local time rather than client fetch time.
...
- 범위를 검증하는 토폴로지 소유권과 인벤토리 근거를 제공합니다. 기존 페이지와 저장 그래프 생성기는 리전·VPC별 대상 확인에 서브넷 인벤토리를 사용합니다. ECS 후보는 RUNNING이어야 하며 EKS 후보는 할당된 IP가 있는 고유한 Pending/Running Pod와 정상 Endpoint 조회로 확인합니다. STOPPED/DELETED 태스크와 Succeeded/Failed Pod는 재사용된 주소를 점유한다고 보지 않으며, 상태 미확인·누락이나 해결되지 않은 Pod 참조는 미확인으로 유지합니다. Pod가 아닌 수동 Service는 다른 네임스페이스의 Pod 소유권이나 이름을 바꾸지 않습니다. 읽을 수 없는 EKS 범위는 해당 IP 전체를 차단하고 다른 범위는 유지하며, 범위 미확인이나 목록 제한은 전체 맵을 차단합니다. 호스트 검증은 보고된 EKS 리전으로 제한합니다. 멤버·전체 계정의 EKS 미조회는 배너와 대상 상세에 표시합니다. 같은 리전·VPC의 두 클러스터에서 같은 IP가 나오면 워크로드 이름이 같아도 해당 IP를 미확인으로 남기며 다른 범위의 주소는 독립적으로 처리합니다. 다른 리전의 후보는 별도 미확인 이름·상세·범위 경고로 보존하며 배타적 클러스터 필터에 쓰지 않습니다. 멤버·저장 그래프의 이름은 실제 소유권이 아닌 캐시된 구성입니다. 대상 노드의 targetCapturedAt은 대상 그룹 행의 수집 시각만 나타내며 태스크·서브넷·Pod의 소유권 시각으로 사용하지 않습니다. 유효하지 않거나 없는 값은 미확인으로 유지하고 호스트 ECS 스냅샷 이름도 캐시된 구성으로 표시합니다. SQL-reader는 region·cluster·ECS·task 식별자를 노출할 수 있지만 VPC·서브넷과 ownership_evidence·targetCapturedAt은 제외하므로 현재 소유권의 증거로 사용할 수 없습니다. 대상 그룹·ECS·서브넷은 제한된 페이지 조회와 전역 수집 버전 확인을 유지하고 EKS와 중단 제한 시간을 공유합니다. 각 인벤토리 페이지와 전역 수집 원장의 행 수를 읽기 전용 Repeatable Read 트랜잭션에서 조회하며 중요 타입은 명시적인 일관성 표시를 요구합니다. 브라우저 시계와 비교하지 않으며 여러 페이지·타입 전체의 단일 스냅샷이나 최신성을 보장하지 않습니다. 부분·실패·실행 중인 수집의 캐시 행은 배타적 소유권을 주장하지 않습니다. 일부 인벤토리 실패 시 성공한 타입은 보존하며 실패·불완전한 새로고침 결과가 빈 그래프이면 같은 계정의 이전 비어 있지 않은 구성과 당시 근거 안내를 유지합니다. 완전한 빈 결과는 정상적으로 교체합니다. 스냅샷 일치만으로 실제 식별 정보를 증명하지 않습니다.
...
- DNS 보류 배포: 같은 브랜치·SHA의 명시적 저장 계획으로 기존 인증서 소유권과 서비스 레코드를 보존하고, 계정 전체 검색 없이 운영자가 지정했거나 이미 연결된 외부 인증서를 검증합니다. 모든 공용·사설 DNS 변경 및 일반 배포의 검증 CNAME 삭제·교체를 차단하고, 빌드 전 ECR 확인과 서비스 DNS 게시 전 Host·SNI를 유지하는 공통 스모크 테스트를 제공합니다. 개발 환경의 선택적 검증은 유효 자격증명을 비공개로 평가하고 실제 Cognito 로그인과 인증된 DB 응답까지 확인합니다. 승인된 런타임 활성화는 CI 대상 계정을 검사하고 사설 DNS를 소유 런타임 리소스로 제한하며 인벤토리·워커 이미지 고정과 저장 계획에 결합된 Lambda asset 검증을 제공합니다. 이 검사만으로 수집 성공을 확정하지 않습니다. 비공개 저장 plan 검토는 실행·SHA와 서명된 asset을 인증한 뒤 보호된 파일로 렌더링합니다. 명시적 plan/apply 실패는 인증된 시도별 복구를 지원하는 제한된 암호화 로그 끝부분을 보존합니다. 평문 임시 파일 없이 표준 입력으로 암호화하며 업로드 성공이 확인된 경우에만 로컬 암호문을 삭제합니다. 자식 프로세스는 AWS STS 자격증명을 유지하되 GitHub 명령 채널과 Terraform 로그·인수 환경변수를 제외하며, Linux에서 첫 취소는 정상 종료를 요청하고 두 번째 취소는 프로세스 그룹을 종료하며, 캡처 부모가 종료되면 Terraform도 종료합니다. 원문 대신 고정 진단 분류·작업 수를 보고하고 명령 종료 상태와 apply gate는 변경하지 않습니다.
...
- 구성 토폴로지 근거: ECS/EKS IP 표시는 별도로 확인된 리전·VPC·서브넷 또는 Pod 근거가 있을 때만 연결한다. 저장된 계정 범위를 읽은 뒤 조회하고 이전 범위의 늦은 응답을 차단하며 실패·응답 상한을 표시한다. 전체 계정 집계 수집 상태를 모든 범위에서 조회 실패·개별 계정 상태 미확인과 구분해 표시하며 실행 중 동기화와 일반적인 Pod IP 모호성을 실패로 취급하지 않는다. RUNNING ECS 태스크만 소유 근거로 사용하여 중지된 태스크가 재사용 IP를 점유하거나 모호하게 만들지 않는다. EKS 소유 근거는 호스트 단독 범위의 API 설정 리전에서만 조회하며 다른 리전과 목록에 포함된 미연결 클러스터를 미평가로 표시하고 연결 준비 상태를 실패·부분 조회와 구분한다. 인벤토리는 계정 선택만 적용하며 빈 그래프에서도 응답 상한을 표시한다. 수집·최근 성공 시각은 클라이언트 조회 시각 대신 원본 근거를 사용하고 사용자 시간대로 표시한다.
