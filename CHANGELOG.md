...
- Account onboarding: entering an Account ID generates copyable AWS CLI commands and a self-contained role-creation script download with the live host role, an automatic ExternalId, optional CLI profile and wrong-account guard. The page guides CloudShell execution and verified registration, preserves retry inputs and displays host-only restrictions before registration.
...
- 계정 온보딩: Account ID 입력 시 실제 호스트 역할·자동 ExternalId·선택한 CLI 프로필을 반영한 AWS CLI 명령어와 독립 실행 가능한 역할 생성 스크립트를 제공합니다. CloudShell 실행부터 검증 후 등록까지 안내하고, 잘못된 계정에서의 실행을 차단하며 재시도 입력값과 호스트 전용 등록 제한을 표시합니다.
