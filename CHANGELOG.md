...
- VPC connectivity: `/inventory/vpc` adds an on-demand, account/region-scoped peering and TGW attachment viewer, a section shortcut and resource-graph navigation, with peer identity/state, route-table associations and localized guidance. Shows the VPC owner or unknown with shared-VPC guidance; missing or different ownership keeps evidence incomplete and prevents a definitive no-connections claim. Only complete results are cached for four minutes; partial-result retries read AWS again, and lookup timestamps reflect read completion. Discloses the 500-row VPC choice cap and incomplete reads; configuration does not prove reachability. Requires a reviewed saved-plan apply for the region-conditioned `ec2:DescribeVpcPeeringConnections` grant; reuses the existing TGW read permission.
...
- VPC 간 연결: `/inventory/vpc`에 사용자 클릭으로 계정·리전 범위의 피어링과 TGW 어태치먼트를 조회하는 화면, 섹션 바로가기와 리소스 그래프 이동을 추가합니다. 상대 VPC 식별 정보·상태, 라우트 테이블 연결과 다국어 안내를 제공합니다. VPC 소유 계정 또는 미확인 상태와 공유 VPC 안내를 표시하며, 소유자가 없거나 조회 계정과 다르면 근거를 불완전하게 유지하여 연결이 없다고 단정하지 않습니다. 완전한 결과만 4분간 캐시하고, 부분 결과를 재시도하면 AWS를 다시 조회하며 조회 시각은 읽기 완료 시점을 나타냅니다. VPC 선택 목록 500행 상한과 불완전한 조회를 명시하고, 연결 구성만으로 실제 통신 가능 여부를 확정하지 않습니다. 리전 조건이 적용된 `ec2:DescribeVpcPeeringConnections` 권한은 검토된 저장 계획으로 apply해야 하며, 기존 TGW 읽기 권한을 재사용합니다.
