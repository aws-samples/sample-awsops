...
- Custom-agent policy enforcement: read current catalog/space settings per turn, retain explicit deny-all policies across revocation, qualify gateway tool identities, and reject reserved routing names. A failed final enablement read leaves an explicit custom pin unavailable; automatic routing removes custom candidates and visibly persists the independent built-in fallback. Product help bypasses that final custom query, and successful deny-all remains deny-all. Existing reserved-name rows no longer route; saved caps can become deny-all. Restriction history persists through skill edits/detachment; integration-only agents gain no gateway tools. Apply the new agent-tool-policy-history migration through the standalone reviewed flow before Web deployment. Initial chat/customization policy-read failures return 503; only hybrid built-in pins bypass the initial chat failure.
...
- 커스텀 에이전트 정책 적용: 매 턴 현재 카탈로그·스페이스 설정을 읽고, 권한 철회 뒤에도 명시적 전체 거부를 유지하며, 게이트웨이 도구 식별자를 한정하고 예약된 라우팅 이름을 거부한다. 최종 활성화 조회 실패 시 명시적 커스텀 지정은 미가용으로 반환하며, 자동 라우팅은 커스텀 후보를 제거하고 별도로 판단한 기본 경로 사용을 표시·저장한다. 제품 도움말은 이 최종 커스텀 조회를 건너뛰며 정상 조회된 전체 거부 정책은 그대로 유지한다. 기존 예약 이름 행은 라우팅되지 않으며 저장된 cap은 deny-all이 될 수 있습니다. 스킬 수정·분리 후에도 제한 이력을 유지하고 연동 전용 에이전트에 게이트웨이 도구를 추가하지 않습니다. Web 배포 전 agent-tool-policy-history migration을 검토된 별도 경로로 적용하세요. 초기 chat/customization 정책 조회 실패는 503이며, chat의 초기 실패 예외는 hybrid 기본 pin뿐입니다.
