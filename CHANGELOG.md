...
- 저장된 토폴로지 근거: 크기가 제한된 인벤토리 스냅샷과 flow/infra/trace의 원자적 발행으로 수집 실패·불가 시 기존 그래프를 보존하고, 멤버 소스 실패와 발행·저하·보존·건너뜀 결과를 명시합니다. 인프라·리소스 페이지는 수집 상태, 확인된 빈 결과, 접이식 소스 상세를 표시하며 캔버스 공간을 유지합니다. 재구축 CLI는 발행 시 0, 보존·건너뜀 시 2, 예외 시 1을 반환하고 웹과 리더는 같은 최신성 설정을 사용합니다.
