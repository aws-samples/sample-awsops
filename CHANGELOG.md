...
- Scoped topology ownership and inventory evidence. The existing topology page and materializer supply subnet inventory for region/VPC-scoped targets. ECS candidates require RUNNING; EKS candidates require an independently listed unique Pending/Running pod with an assigned IP and valid endpoint read. STOPPED tasks and Succeeded/Failed pods do not claim reused addresses; unknown phases/states and unresolved pod references remain unverified. Manual non-pod Service entries do not contest or rename pods across namespaces. Unreadable EKS scopes block every matching IP while unrelated scopes remain usable; unknown scope or listing limits block the full map. Host checks cover only the reported EKS region. Other-region candidates retain separate unverified labels, details and a coverage banner without exclusive cluster filtering. Member/materialized labels remain cached configuration, not live ownership. Target inventory capture times stay supplied or unknown, without temporal-equivalence claims. SQL-reader can expose bare region/cluster/ECS/task identifiers while omitting VPC/subnet, ownership_evidence and target capturedAt; those identifiers do not establish live ownership. ECS/subnet reads keep bounded paging, stable global sweep metadata and a shared abort deadline with EKS. Partial/failed/running sweeps retain cached rows without exclusive ownership. Partial inventory reads preserve successful types; a failed refresh retains only prior same-account configuration and its provenance notices. Snapshot agreement never independently proves live identity.
...
- 범위를 검증하는 토폴로지 소유권과 인벤토리 근거를 제공합니다. 기존 페이지와 저장 그래프 생성기는 리전·VPC별 대상 확인에 서브넷 인벤토리를 사용합니다. ECS 후보는 RUNNING이어야 하며 EKS 후보는 할당된 IP가 있는 고유한 Pending/Running Pod와 정상 Endpoint 조회로 확인합니다. STOPPED 태스크와 Succeeded/Failed Pod는 재사용된 주소를 점유한다고 보지 않으며, 상태 미확인·누락이나 해결되지 않은 Pod 참조는 미확인으로 유지합니다. Pod가 아닌 수동 Service는 다른 네임스페이스의 Pod 소유권이나 이름을 바꾸지 않습니다. 읽을 수 없는 EKS 범위는 해당 IP 전체를 차단하고 다른 범위는 유지하며, 범위 미확인이나 목록 제한은 전체 맵을 차단합니다. 호스트 검증은 보고된 EKS 리전으로 제한합니다. 다른 리전의 후보는 별도 미확인 이름·상세·범위 경고로 보존하며 배타적 클러스터 필터에 쓰지 않습니다. 멤버·저장 그래프의 이름은 실제 소유권이 아닌 캐시된 구성입니다. 대상 인벤토리 수집 시각은 제공된 값이나 미확인으로 유지하며 시간적 동등성을 주장하지 않습니다. SQL-reader는 region·cluster·ECS·task 식별자를 노출할 수 있지만 VPC·서브넷과 ownership_evidence·대상 capturedAt은 제외하므로 현재 소유권의 증거로 사용할 수 없습니다. ECS·서브넷은 제한된 페이지 조회와 전역 수집 버전 확인을 유지하고 EKS와 중단 제한 시간을 공유합니다. 부분·실패·실행 중인 수집의 캐시 행은 배타적 소유권을 주장하지 않습니다. 일부 인벤토리 실패 시 성공한 타입은 보존하며 전체 새로고침 실패 시 같은 계정의 이전 구성과 당시 근거 안내만 유지합니다. 스냅샷 일치만으로 실제 식별 정보를 증명하지 않습니다.
