...
- 토폴로지 서비스·네트워크 보기(`/topology?view=e2e`): 구성된 진입 경로, 저장된 서비스 관측, Network Flow Monitor 트래픽을 범위가 확인된 엔드포인트 식별자로 연결한다. 모니터·메트릭·기간을 명시적으로 조회해 전송량·RTT·재전송·타임아웃·NAT 주소·경유 구성요소를 확인하며, 구성·서비스·네트워크·식별자·문맥 관계를 구분한다. 검색과 주요 흐름 확대, 수집 시각·부분 결과 안내를 제공하고 하나의 추적된 요청이나 순서가 확인된 패킷 경로로 단정하지 않는다. 관측 결합은 호스트·기본 리전 범위이며 계정 전환 시 이전 결과를 버리고 EKS 사설 IP는 리전·VPC 범위로 식별한다. 기존 Network Flow와 서비스 맵에서 바로 이동할 수 있다.
