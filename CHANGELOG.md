...
- PR review pipeline: chair timeout 600→900s with a one-shot fast-fail retry, code-first three-pass diff ordering (code → decisions/runbooks → docs prose), and a sanitized, narrowly-scoped truncated-file guard that downgrades unverifiable absence-claims to MINOR instead of dropping them. Each L2–L5 lens requires completed Codex and Claude reviews; every Codex and Claude panel lens has a 1200s budget, the job is bounded at 90 minutes, and the same OIDC role obtains fresh credentials immediately before each model phase after setup. Failed or timed-out panel/chair output cannot count as a completed review, with hard-kill grace retained. Lockfile change metadata remains visible when generated contents are omitted. A SHA-pinned recovery label selects a commit, while a GitHub protected environment requires a designated reviewer before execution; exact environment/ref trust subjects replace coarse PR/repository trust. Automatic CI scripts come from the trusted default commit, model context comes from the target base, and stale verdicts are not published. Changed static PNGs are staged from immutable HEAD Git blobs with bounded read-only manifests for every panelist and the chair; BASE pixels remain historical context, and unavailable required image evidence fails coverage without bypassing findings or review gates. Bounded report validation requires explicit completed image coverage from every cell and the chair when images are staged; missing or failed declarations override PASS and are labeled as incomplete review, while quoted examples do not count. Valid PNGs survive unsupported/over-limit entries, with explicit published coverage failures instead of abandoned reviews. Safe prompt labels accompany hash-checked Codex image attachments and Claude Read paths; no tool privileges are added. See `docs/runbooks/pr-review-head-images.md`.
