...
- 구성 토폴로지 근거: ECS/EKS IP 표시는 별도로 확인된 리전·VPC·서브넷 또는 Pod 근거가 있을 때만 연결한다. 저장된 계정 범위를 읽은 뒤 조회하고 이전 범위의 늦은 응답을 차단하며 실패·응답 상한을 표시한다. 전체 계정 집계 수집 상태를 모든 범위에서 조회 실패·개별 계정 상태 미확인과 구분해 표시하며 실행 중 동기화와 일반적인 Pod IP 모호성을 실패로 취급하지 않는다. RUNNING ECS 태스크만 소유 근거로 사용하여 중지된 태스크가 재사용 IP를 점유하거나 모호하게 만들지 않는다. EKS 소유 근거는 호스트 단독 범위에서만 조회하고 다른 범위에는 미시도를 명시하며 실패·부분 조회를 구분한다. 수집·최근 성공 시각은 클라이언트 조회 시각 대신 원본 근거를 사용하고 사용자 시간대로 표시한다.
