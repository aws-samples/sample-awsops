...
describe('bounded ownership inventory paging', () => {
  ...
  it.each(['target_group', 'ecs_task', 'subnet'].flatMap(type => [undefined, null, 'read-committed'].map(consistency => ({ type, consistency }))))(
    'withholds ownership without verified snapshot consistency for $type / $consistency', async ({ type, consistency }) => {
      ...
      render(<TopologyPage />);
      await screen.findByText(new RegExp(`${type}: invalid inventory response`));
      expect(requests.filter(u => u.pathname.endsWith(`/${type}`))).toHaveLength(1);
      expect(screen.queryByRole('option', { name: 'ECS · ecs-app' })).toBeNull();
      expect(screen.queryByRole('option', { name: 'EKS · good' })).toBeNull();
    });

  it.each([-86400000, 86400000])('verified database snapshots are independent of browser clock skew: %sms', async delta => {
    vi.spyOn(Date, 'now').mockReturnValue(Date.parse(RUN.finished_at) + delta);
    serve(); render(<TopologyPage />);
    await screen.findByRole('option', { name: 'EKS · good' });
    expect(screen.queryByText('인벤토리 동기화가 완료되지 않아 IP 소유권을 확인할 수 없습니다.')).toBeNull();
  });
