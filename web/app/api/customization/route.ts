...
import {
  validateToolBindings, upsertSkill, upsertAgent, attachSkill, setEnabled, listAgentsWithSkills, listSkills, writeAudit,
} from '@/lib/catalog';
...
async function bindingError(change: Parameters<typeof validateToolBindings>[0]) {
  try {
    const errors = await validateToolBindings(change);
    return errors.length ? json({ error: 'invalid tool policy', detail: errors }, 400) : null;
  } catch { return json({ error: 'Tool policy validation unavailable' }, 503); }
}
...
export async function POST(request: Request) {
  ...
  if (body.kind === 'skill') {
    ...
    const invalid = await bindingError({ skillName: String(body.name), tools: body.toolAllowlist as string[] });
  ...
  if (body.kind === 'agent') {
    ...
    const invalid = await bindingError({ agentName: String(body.name), gateway: String(body.gateway) });
    if (invalid) return invalid;
    let id: number;
    try {
...
export async function PUT(request: Request) {
  ...
  if (body.op === 'attach') {
    const invalid = await bindingError({ agentId: Number(body.agentId), skillId: Number(body.skillId) });
    if (invalid) return invalid;
