...
// Reuse the same bound values in every UNION arm and aggregate query.
// null means all; an explicit empty region list must remain an empty selection.
function accountValues(accounts: '__all__' | string[]): string[] | null {
  if (accounts === '__all__') return null;
  const safe = accounts.filter(a => a === 'self' || /^[0-9]{12}$/.test(a));
  return safe.length ? safe : ['self'];
}

function regionValues(regions: '__all__' | string[], includeGlobal: boolean): string[] | null {
  if (regions === '__all__') return null;
  const allowed = regions.filter(r => r !== 'global' && /^[a-z0-9-]{1,32}$/.test(r));
  return includeGlobal ? [...allowed, 'global'] : allowed;
}

const SCOPE_SQL = `($1::text[] IS NULL OR account_id = ANY($1::text[]))
  AND ($2::text[] IS NULL OR region = ANY($2::text[]))
  AND ($3::boolean OR region <> 'global')`;
...
const splitsSql = (): string => `
  SELECT 'ec2_running' AS k, count(*)::int AS n FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='ec2' AND data->>'instance_state'='running'
  UNION ALL SELECT 'ec2_stopped', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='ec2' AND data->>'instance_state'='stopped'
  UNION ALL SELECT 'ebs_unencrypted', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='ebs_volume' AND (data->>'encrypted')='false'
  UNION ALL SELECT 'iam_user_no_mfa', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='iam_user' AND (data->>'mfa_enabled')='false'
  UNION ALL SELECT 'sg_open_ingress', count(*)::int FROM inventory_resources
    WHERE ${SCOPE_SQL} AND resource_type='security_group'
    AND (data->'ip_permissions')::text ~ '"(cidr_ip|CidrIp|cidr_ipv6|CidrIpv6)"\\s*:\\s*"(0\\.0\\.0\\.0/0|::/0)"'
  UNION ALL SELECT 's3_public', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='s3_public_access' AND ${PUBLIC_S3_WHERE}
  UNION ALL SELECT 'cw_alarm', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='cloudwatch_alarm' AND lower(data->>'state_value')='alarm'
  UNION ALL SELECT 'lambda_runtimes', count(DISTINCT COALESCE(NULLIF(data->>'runtime',''),'custom'))::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='lambda'
  UNION ALL SELECT 'lambda_long_timeout', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='lambda' AND (data->>'timeout') ~ '^[0-9]+$' AND (data->>'timeout')::int > 300
  UNION ALL SELECT 'ebs_total_gb', COALESCE(sum(CASE WHEN (data->>'size') ~ '^[0-9]+$' THEN (data->>'size')::int ELSE 0 END),0)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='ebs_volume'
  UNION ALL SELECT 'rds_multi_az', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='rds' AND (data->>'multi_az')='true'
  UNION ALL SELECT 'rds_unencrypted', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='rds' AND (data->>'storage_encrypted')='false'
  UNION ALL SELECT 'ecr_scan_on_push', count(*)::int FROM inventory_resources
    WHERE ${SCOPE_SQL} AND resource_type='ecr'
    AND (data->'image_scanning_configuration')::text ~* '"(scan_on_push|ScanOnPush)"\\s*:\\s*(true|"true"|1)'
  UNION ALL SELECT 'ecr_immutable', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='ecr' AND upper(data->>'image_tag_mutability')='IMMUTABLE'
  UNION ALL SELECT 's3_versioning_off', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='s3' AND (data->>'versioning_enabled')='false'
  UNION ALL SELECT 'cloudfront_enabled', count(*)::int FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='cloudfront' AND (data->>'enabled')='true'
`;
...
export async function GET(request: Request) {
  ...
  const scope = [accountValues(accounts), regionValues(regions, includeGlobal), includeGlobal];
  try {
    const pool = getPool();
    ...
    const r = await pool.query<{ resource_type: string; n: number }>(
      `SELECT resource_type, count(*)::int AS n FROM inventory_resources
       WHERE ${SCOPE_SQL} GROUP BY resource_type`, scope,
    );
    ...
    let splitsOk = true;
    try {
      const sr = await pool.query<{ k: string; n: number }>(splitsSql(), scope);
    ...
    try {
      const er = await pool.query<{ t: string; n: number }>(
        `SELECT COALESCE(NULLIF(data->>'instance_type',''),'unknown') AS t, count(*)::int AS n
         FROM inventory_resources WHERE ${SCOPE_SQL} AND resource_type='ec2'
         GROUP BY 1 ORDER BY n DESC LIMIT 10`, scope,
      );
