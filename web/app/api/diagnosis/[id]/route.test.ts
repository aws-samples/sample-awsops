...
describe('GET /api/diagnosis/[id]', () => {
  it('401 when unauthenticated', async () => {
    ...
    expect((await GET(req(), { params: Promise.resolve({ id: '1' }) })).status).toBe(401);
  });
  it('400 on a non-numeric id', async () => {
    expect((await GET(req(), { params: Promise.resolve({ id: 'abc' }) })).status).toBe(400);
  });
  it('404 for missing', async () => {
    expect((await GET(req(), { params: Promise.resolve({ id: '999' }) })).status).toBe(404);
  });
  it('returns the report with can_edit', async () => {
    const r = await GET(req(), { params: Promise.resolve({ id: '1' }) });
  ...
  it('403 for a non-owner, non-admin (read path is now gated, not just can_edit)', async () => {
    ...
    expect((await GET(req(), { params: Promise.resolve({ id: '1' }) })).status).toBe(403);
  });
});

describe('PATCH /api/diagnosis/[id]', () => {
  it('200 for owner/admin and calls updateReportMeta (sanitized)', async () => {
    const r = await PATCH(req({ title: '  핵심 제목  ', tags: ['보안', '보안', ' 비용 '] }), { params: Promise.resolve({ id: '1' }) });
    expect(r.status).toBe(200);
  ...
  it('403 for a stranger', async () => {
    (canMutateReport as any).mockResolvedValue(false);
    expect((await PATCH(req({ title: 'x' }), { params: Promise.resolve({ id: '1' }) })).status).toBe(403);
    expect(updateReportMeta).not.toHaveBeenCalled();
  });
  it('401 unauthenticated', async () => {
    (verifyUser as any).mockResolvedValue(null);
    expect((await PATCH(req({ title: 'x' }), { params: Promise.resolve({ id: '1' }) })).status).toBe(401);
  });
  it('404 missing report', async () => {
    expect((await PATCH(req({ title: 'x' }), { params: Promise.resolve({ id: '999' }) })).status).toBe(404);
  });
  it('400 when tags is not an array', async () => {
    expect((await PATCH(req({ tags: 'nope' }), { params: Promise.resolve({ id: '1' }) })).status).toBe(400);
  });
  it('400 when body has no title/tags', async () => {
    expect((await PATCH(req({ foo: 1 }), { params: Promise.resolve({ id: '1' }) })).status).toBe(400);
  });
});

describe('DELETE /api/diagnosis/[id]', () => {
  it('200 for owner/admin and soft-deletes', async () => {
    const r = await DELETE(req(), { params: Promise.resolve({ id: '1' }) });
    expect(r.status).toBe(200);
    expect(softDeleteReport).toHaveBeenCalledWith(1);
  });
  it('403 for a stranger', async () => {
    (canMutateReport as any).mockResolvedValue(false);
    expect((await DELETE(req(), { params: Promise.resolve({ id: '1' }) })).status).toBe(403);
