...
export async function POST(request: Request, { params }: { params: { cluster: string } }) {
  try {
    ...
    // Only metadata discovery assumes the member role; Access Entry checks use the
    // HOST task-role principal. No AWS resource is created or changed by this route.
    ...
    try {
      ...
      /* empty/invalid body OK — preserve the existing task-role default */
    }
