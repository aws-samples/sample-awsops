...
const { query, invoke } = vi.hoisted(() => ({ query: vi.fn(), invoke: vi.fn() }));
...
vi.mock('@/lib/chat-store', () => ({ recordExchange: async () => {} }));
...
async function chat(section?: string) {
  const response = await POST(new Request('http://localhost/api/chat', {
    method: 'POST', headers: { 'content-type': 'application/json', cookie: 'awsops_token=test' },
    body: JSON.stringify({ prompt: 'IAM users', section, lang: 'en' }),
  }));
...
describe('final custom-agent enablement availability', () => {
  it.each([undefined, 'compliance'])('stops dispatch when only the final query fails (pin=%s)', async section => {
    enablement = 'unavailable';
    const { response, text } = await chat(section);
    expect(response.status).toBe(503);
    expect(reads).toEqual(['space', 'catalog', 'enablement']);
    expect(JSON.parse(text)).toEqual({ error: 'Custom-agent policy unavailable' });
