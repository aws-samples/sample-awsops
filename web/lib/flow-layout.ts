 ...
 * whatever width/height the caller actually renders that node at (e.g. PolicyGraph.tsx; also the planned E2E canvas), or dagre's
...
export function layoutFlow(
  ...
  for (const n of graph.nodes) {
    ...
    // Dagre adds coordinates to labels; callers may share an immutable size object.
    g.setNode(n.id, { ...size });
  }
