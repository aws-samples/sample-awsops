...
## Key Files
- `topology/E2eGraphCanvas.tsx` — reusable, unwired evidence canvas for the pure E2E graph; preserves the search query and evidence filters across filter changes and clears invalid or explicitly reset selection and distinguishes configuration, service, network, identity and context edges. No source fetching. Details respect evidence filters independently of canvas caps and disclose bounded-list omissions. Shared main-flow ranking precedes display bounds and compares only one metric/unit group; omitted categories are disclosed. Unsupported or absent observations do not render zero-match counters. Context includes traversed constructs and cached records.
