...
## Key Files
- `topology/E2eGraphCanvas.tsx` — reusable, unwired evidence canvas for the pure E2E graph; preserves query preferences and clears invalid or explicitly reset selection and distinguishes configuration, service, network, identity and context edges. No source fetching. Details respect evidence filters independently of canvas caps and disclose bounded-list omissions. Main-flow ranking compares values only within one metric/unit group.
...
- `../lib/i18n-terms.ts` pre-registers the follow-up Service + Network source-panel strings alongside the reusable canvas labels; the unwired prerequisite intentionally does not consume those panel strings yet.
