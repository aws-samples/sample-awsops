...
## Key Files
- `topology/E2eGraphCanvas.tsx` — reusable evidence canvas for the pure E2E graph; preserves filter/search/focus state and distinguishes configuration, service, network, identity and context edges. No source fetching. `ServiceNetworkTopology.tsx` independently loads the host trace/NFM sources, preserves collection metadata and safe auth/error states, and executes explicit network queries.
